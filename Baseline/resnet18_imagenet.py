...
def resnet18_imagenet() -> nn.Module:
    """Standard torchvision ResNet-18 for ImageNet (3-channel input, 1000 classes)."""
    return resnet18(weights=None, num_classes=1000)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PyTorch ImageNet ResNet-18 baseline (no perforation).")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=64,
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="resnet18_imagenet_last.pt",
        help="Filename for last checkpoint (inside output-dir).",
    )
    parser.add_argument(
        "--best-model-path",
        type=str,
        default="resnet18_imagenet_best.pt",
    ...
    if args.save_model:
        torch.save(model.state_dict(), "imagenet_resnet18_baseline.pt")


if __name__ == "__main__":
    main()

# Standard ResNet-18 ImageNet baseline for comparison against perforated runs