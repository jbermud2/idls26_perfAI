...
def save_training_checkpoint(
    ...
    seconds_per_training_cycle: float,
    run=None,
) -> None:
    checkpoint = {
        "epoch": int(epoch),
    ...
    if run is not None:
        try:
            ...
            print(f"W&B checkpoint upload failed: {exc}")
...
def load_training_checkpoint(
    ...
    if not os.path.exists(resolved_path) and run is not None and wandb is not None:
        try:
            ...
            print(f"W&B checkpoint restore failed: {exc}")
    ...
    try:
        ...
        print(
            "Loaded checkpoint from {} (epoch {}).".format(
                resolved_path,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PyTorch CNN MNIST baseline (no perforation)."
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    if args.resume_from_checkpoint:
        ...
        if checkpoint is not None:
            try:
                ...
                print(f"Checkpoint is missing required key {exc}. Starting from scratch.")
                start_epoch = 1
...
# Regular CNN MNIST baseline for comparison against perforated CNN runs
