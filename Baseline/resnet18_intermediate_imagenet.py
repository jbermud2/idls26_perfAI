...
class WidenedResNet18ImageNet(nn.Module):
    """
    ResNet-18 layout [2,2,2,2] with configurable stage widths and standard ImageNet stem.
    """

    def __init__(
        self,
        layers: Tuple[int, int, int, int] = (2, 2, 2, 2),
        channels: Tuple[int, int, int, int] = DEFAULT_WIDE_CHANNELS,
        num_classes: int = 1000,
    ) -> None:
        super().__init__()
        if len(layers) != 4 or len(channels) != 4:
            raise ValueError("layers and channels must each have length 4")
...
def resnet18_widened_imagenet(
    channels: Tuple[int, int, int, int] = DEFAULT_WIDE_CHANNELS,
) -> nn.Module:
    """Widened ResNet-18 family model for ImageNet (1000 classes)."""
    return WidenedResNet18ImageNet(layers=(2, 2, 2, 2), channels=channels, num_classes=1000)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PyTorch ImageNet widened ResNet-18 intermediate baseline (no perforation)."
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="resnet18_intermediate_imagenet_last.pt",
        help="Filename for last checkpoint (inside output-dir).",
    )
    parser.add_argument(
        "--best-model-path",
        type=str,
        default="resnet18_intermediate_imagenet_best.pt",
    ...
    if args.save_model:
        torch.save(model.state_dict(), "imagenet_resnet18_intermediate_baseline.pt")


if __name__ == "__main__":
    main()

# Widened ResNet-18 ImageNet intermediate baseline for comparison against perforated runs