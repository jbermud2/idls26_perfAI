"""Plain EfficientNet baseline training (registered models, e.g. `--model efficientnet_b5`)."""
...
def main():
    ...
    parser.add_argument("--lr", type=float, default=model_defaults.get("lr", 1e-3), metavar="LR")
    parser.add_argument("--weight-decay", type=float, default=model_defaults.get("weight_decay", 1e-4), metavar="WD")
    parser.add_argument(
        "--finetune-backbone",
        action=argparse.BooleanOptionalAction,
        default=model_defaults.get("finetune_backbone", True),
        help="Fine-tune pretrained backbone (default: True). Use --no-finetune-backbone for head-only.",
    )
