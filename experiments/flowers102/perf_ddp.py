...
# RESTRUCTURE_FLAG_FILENAME = ".pai_restructure_needed"


# def _restructure_flag_path(output_dir: str) -> str:
#     return os.path.join(output_dir, RESTRUCTURE_FLAG_FILENAME)


# def _write_restructure_flag(output_dir: str, epoch: int, pai_system_name: str) -> None:
#     import json
#     flag = {"epoch": epoch, "pai_system_name": pai_system_name}
#     with open(_restructure_flag_path(output_dir), "w") as f:
#         json.dump(flag, f)


# def _read_and_clear_restructure_flag(output_dir: str) -> Optional[dict]:
#     import json
#     path = _restructure_flag_path(output_dir)
...
#                         help="number of epochs to train (default: 0 = PAI-controlled)")
...
#     parser.add_argument("--checkpoint-path", type=str, default="efficientnet_b5_flowers102_last.pt",
#                         help="filename for last checkpoint (inside output-dir)")
#     parser.add_argument("--best-model-path", type=str, default="efficientnet_b5_flowers102_best.pt",
...
#     parser.add_argument("--output-dir", type=str, default="outputs_flowers_efficientnet",
...
#                         choices=["allow", "must", "never"], help="W&B resume behavior")
#     parser.add_argument("--wandb-anonymous", type=str, default="never",
#                         choices=["never", "allow", "must"], help="W&B anonymous mode")
...
#     parser.add_argument("--pai-forward-function", type=str, default="relu",
#                         choices=["relu", "sigmoid", "tanh"],
#                         help="Forward function for PerforatedAI added nodes")
#     parser.add_argument("--pai-convert-target", type=str, default=DEFAULT_PAI_CONVERT_TARGET,
#                         choices=["pre_fc", "classifier_fc"],
#                         help="Module to convert with dendrites.")
#     parser.add_argument("--perforated-load-path", type=str, default="",
#                         help="Optional saved PerforatedAI system name/path to load")
#     parser.add_argument("--pai-save-name", type=str, default="efficientnet_b5_flowers102_pai",
...
#                         help="keep PerforatedAI unwrapped-module interactive checks enabled")
#     parser.add_argument("--strict-weight-decay-check", action="store_true", default=False,
#                         help="keep PerforatedAI weight-decay interactive check enabled")
#     parser.add_argument("--debug-pai-switch-logs", action="store_true", default=False,
#                         help="show raw PerforatedAI output around add_validation_score")
...
#     # Auto-resume after a PAI restructure exit
#     flag = _read_and_clear_restructure_flag(os.path.abspath(args.output_dir))
#     if flag is not None:
#         print(f"Detected PAI restructure flag from epoch {flag['epoch']} — auto-resuming from latest PAI checkpoint.")
#         if not args.perforated_load_path:
#             args.perforated_load_path = flag["pai_system_name"]
...
#         raise ImportError(
#             "PerforatedAI is required for this script. Install it from the PerforatedAI source/package."
#         )
...
#             print(f"DDP find_unused_parameters={effective_find_unused} " +
#                   ("(auto-enabled for dendrite mode)" if args.dendrite_mode > 0 and not args.ddp_find_unused_parameters else ""))
#         else:
...
#     train_dataset = datasets.Flowers102(root=args.data_root, split="train",
#                                         transform=train_transform, download=download)
#     val_dataset = datasets.Flowers102(root=args.data_root, split="val",
#                                       transform=val_transform, download=download)
#     test_dataset = datasets.Flowers102(root=args.data_root, split="test",
...
#                                        rank=args.global_rank, shuffle=True,
#                                        seed=args.seed) if is_distributed else None
#     val_sampler = DistributedSampler(val_dataset, num_replicas=args.world_size,
#                                      rank=args.global_rank, shuffle=False,
#                                      seed=args.seed) if is_distributed else None
#     test_sampler = DistributedSampler(test_dataset, num_replicas=args.world_size,
#                                       rank=args.global_rank, shuffle=False,
#                                       seed=args.seed) if is_distributed else None

#     train_loader = torch.utils.data.DataLoader(
#         train_dataset, batch_size=args.batch_size, sampler=train_sampler,
#         shuffle=(train_sampler is None), num_workers=num_workers, pin_memory=use_cuda)
#     val_loader = torch.utils.data.DataLoader(
#         val_dataset, batch_size=args.test_batch_size, sampler=val_sampler,
...
#         val_dataset, batch_size=1, sampler=val_sampler if is_distributed else None,
#         shuffle=False, num_workers=num_workers, pin_memory=use_cuda)
#     gpu_benchmark_loader = torch.utils.data.DataLoader(
#         val_dataset, batch_size=8, sampler=val_sampler if is_distributed else None,
#         shuffle=False, num_workers=num_workers, pin_memory=use_cuda)

#     base_model = efficientnet_b5_flowers102(num_classes=NUM_CLASSES,
...
#             parallel_model, device, val_loader, test_loader,
#             is_main_process=is_main_process,
#         )
...
#                     model_for_pai, pai_system_name, args.finetune_backbone,
...
#             pai_system_name, model_for_pai,
#             is_distributed=is_distributed, is_main_process=is_main_process,
#         )
...
#                     print(f"PAI add_validation_score returned non-fatal SystemExit "
...
#                         print(f"PAI add_validation_score retry returned non-fatal SystemExit "
...
#                     print(f"PAI add_validation_score retry failed; "
...
#         if restructured:
#             if is_main_process:
#                 print(f"PAI restructured at epoch {epoch} — saving and exiting for clean relaunch.")
#                 _write_restructure_flag(args.output_dir, epoch, pai_system_name)
#                 checkpoint_model_state = model_for_pai.state_dict()
#                 torch.save({
#                     "epoch": epoch,
...
#         # Non-restructure path: step the scheduler normally
#         with warnings.catch_warnings():
#             warnings.filterwarnings(
#                 "ignore",
...
#                     save_on_master(
#                         _state_dict_without_pai_metadata(model_to_save,
#                                                          disallowed_module_id=disallowed_module_id),
#                         best_model_path,
#                     )
#                 except Exception as exc:
...
#         save_on_master(model.state_dict(),
#                        os.path.join(args.output_dir, "efficientnet_b5_flowers102_baseline.pt"))
...
For running:
interact -p GPU-shared --gres=gpu:v100-32:2 -t 8:00:00 -A cis260045p
python -m torch.distributed.run --nproc_per_node 2 /ocean/projects/cis260045p/shared/flowers_perforatedai_parallel.py --data-root ./data --use-wandb --wandb-api-key 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 --dendrite-mode 1 --max-dendrites 5 --pai-forward-function relu --improvement-threshold 0.5 --candidate-weight-init-mult 0.1 --epochs 40 > output.txt 2>&1
'''
...
def _resolve_pai_save_name(output_dir: str, save_name: str) -> str:
    """Resolve save_name to a simple relative path for PAI library compatibility."""
    return os.path.normpath(save_name)
...
def broadcast_full_model(model: nn.Module, device: torch.device, src: int = 0) -> nn.Module:
    """Broadcast the entire model (structure + weights) from src rank to all others.

    Unlike broadcast_model_state_dict which only broadcasts weights, this function
    broadcasts the full model including any structural changes (like added dendrites).
    Non-src ranks will receive a completely new model object.
    """
    if not (dist.is_available() and dist.is_initialized()):
        return model

    rank = dist.get_rank()
    model_for_broadcast = model.module if isinstance(model, DDP) else model

    if rank == src:
        buf = io.BytesIO()
        torch.save(model_for_broadcast, buf)
        byte_data = buf.getvalue()
        length_tensor = torch.tensor([len(byte_data)], dtype=torch.long, device=device)
    else:
        length_tensor = torch.tensor([0], dtype=torch.long, device=device)

    dist.broadcast(length_tensor, src=src)
    buf_len = int(length_tensor.item())

    if rank == src:
        byte_tensor = torch.frombuffer(bytearray(byte_data), dtype=torch.uint8).to(device)
    else:
        byte_tensor = torch.zeros(buf_len, dtype=torch.uint8, device=device)

    dist.broadcast(byte_tensor, src=src)

    if rank != src:
        received_bytes = byte_tensor.cpu().numpy().tobytes()
        model_for_broadcast = torch.load(
            io.BytesIO(received_bytes),
            map_location=device,
            weights_only=False,
        )
        print(f"[rank {rank}] Received full model from rank {src}")

    return model_for_broadcast
...
def broadcast_model_state_dict(
    model: nn.Module,
    device: torch.device,
    src: int = 0,
) -> None:
    """
    Broadcast rank `src`'s model state dict to all other ranks in-place.

    Strategy: serialize the state dict to a CPU byte buffer on rank `src`,
    broadcast the buffer length first (so every rank can allocate), then
    broadcast the buffer itself, and finally deserialize + load on non-src ranks.

    This avoids any filesystem I/O and is robust to structural model changes
    (e.g. PAI adding dendrite parameters) because the state dict is transmitted
    as a raw byte stream rather than individual tensors.
    """
    if not (dist.is_available() and dist.is_initialized()):
        return  # Nothing to do in single-process mode.

    rank = dist.get_rank()
    model_for_broadcast = model.module if isinstance(model, DDP) else model

    # --- Step 1: rank 0 serializes the state dict to a byte buffer. ---
    if rank == src:
        buf = io.BytesIO()
        torch.save(model_for_broadcast.state_dict(), buf)
        byte_data = buf.getvalue()
        length_tensor = torch.tensor([len(byte_data)], dtype=torch.long, device=device)
    else:
        length_tensor = torch.tensor([0], dtype=torch.long, device=device)

    # --- Step 2: broadcast the buffer length so every rank can allocate. ---
    dist.broadcast(length_tensor, src=src)
    buf_len = int(length_tensor.item())

    # --- Step 3: broadcast the raw byte buffer. ---
    # Pack into a uint8 tensor for NCCL/Gloo compatibility.
    if rank == src:
        byte_tensor = torch.frombuffer(bytearray(byte_data), dtype=torch.uint8).to(device)
    else:
        byte_tensor = torch.zeros(buf_len, dtype=torch.uint8, device=device)
    ...
    # --- Step 4: non-src ranks deserialize and load. ---
    if rank != src:
        received_bytes = byte_tensor.cpu().numpy().tobytes()
        state_dict = torch.load(io.BytesIO(received_bytes), map_location=device)
        state_dict.pop("tracker_string", None)
        incompatible = model_for_broadcast.load_state_dict(state_dict, strict=False)
        if incompatible.missing_keys or incompatible.unexpected_keys:
            print(
                f"[rank {rank}] broadcast_model_state_dict: non-strict load. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--pai-convert-target", type=str, default=DEFAULT_PAI_CONVERT_TARGET, choices=["pre_fc", "classifier_fc"]
    )
    parser.add_argument("--perforated-load-path", type=str, default="")
    parser.add_argument("--pai-save-name", type=str, default="pai_state")
    parser.add_argument("--strict-unwrapped-check", action="store_true", default=False)
    parser.add_argument("--strict-weight-decay-check", action="store_true", default=False)
    parser.add_argument("--debug-pai-switch-logs", action="store_true", default=False)
    parser.add_argument("--ddp-find-unused-parameters", action="store_true", default=False)
    ...
    download = not args.no_download
    train_dataset = datasets.Flowers102(root=args.data_root, split="train", transform=train_transform, download=download)
    val_dataset = datasets.Flowers102(root=args.data_root, split="val", transform=val_transform, download=download)
    test_dataset = datasets.Flowers102(root=args.data_root, split="test", transform=test_transform, download=download)

    print(f"Dataset sizes -> train: {len(train_dataset)}, val: {len(val_dataset)}, test: {len(test_dataset)}")

    num_workers = args.num_workers if args.num_workers >= 0 else 0

    train_sampler = DistributedSampler(train_dataset, num_replicas=args.world_size, rank=args.global_rank, shuffle=True, seed=args.seed) if is_distributed else None
    val_sampler = DistributedSampler(val_dataset, num_replicas=args.world_size, rank=args.global_rank, shuffle=False, seed=args.seed) if is_distributed else None
    test_sampler = DistributedSampler(test_dataset, num_replicas=args.world_size, rank=args.global_rank, shuffle=False, seed=args.seed) if is_distributed else None

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, sampler=train_sampler, shuffle=(train_sampler is None), num_workers=num_workers, pin_memory=use_cuda)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.test_batch_size, sampler=val_sampler, shuffle=False, num_workers=num_workers, pin_memory=use_cuda)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.test_batch_size, sampler=test_sampler, shuffle=False, num_workers=num_workers, pin_memory=use_cuda)
    cpu_benchmark_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1, sampler=val_sampler if is_distributed else None, shuffle=False, num_workers=num_workers, pin_memory=use_cuda)
    gpu_benchmark_loader = torch.utils.data.DataLoader(val_dataset, batch_size=8, sampler=val_sampler if is_distributed else None, shuffle=False, num_workers=num_workers, pin_memory=use_cuda)
    ...
    if not args.perforated_load_path:
        if is_main_process and os.path.exists(pai_system_name):
            ...
            try:
                ...
                print(f"Warning: could not remove old PAI directory: {e}")
        if is_distributed:
            dist.barrier(device_ids=[int(os.environ["LOCAL_RANK"])])
    ...
    while True:
        ...
        if is_main_process:
            ...
            GPA.pai_tracker.add_extra_score(train_top5_accuracy, "train_top5")
        val_metrics, test_metrics = test(
            parallel_model, device, val_loader, test_loader, is_main_process=is_main_process,
        )
        ...
        if is_main_process:
            if not _model_has_pai_modules(model_for_pai):
                print("Detected model without PAI modules before switch logic; rebuilding PAI switch model.")
                model_for_pai = prepare_pai_switch_model(
                    model_for_pai, pai_system_name, args.finetune_backbone, disallowed_module_id=disallowed_module_id,
                )
                rebuilt_switch_model = True
            _assert_disallowed_module_not_converted(model_for_pai, disallowed_module_id)
            if rebuilt_switch_model:
        ...
        ensure_pai_switch_files_exist(
            pai_system_name, model_for_pai, is_distributed=is_distributed, is_main_process=is_main_process,
        )
        ...
        if is_main_process:
            try:
                ...
                if _is_nonfatal_pai_system_exit(exc):
                    print(f"PAI add_validation_score returned non-fatal SystemExit (code={exc.code!r}); continuing.")
                else:
                    raise
            except Exception as exc:
                print(f"PAI add_validation_score failed once: {exc}. Attempting recovery and single retry.")
        ...
        if is_distributed:
            # ----------------------------------------------------------------
            # Broadcast control flags from rank 0 to all other ranks so every
            # rank knows whether a restructure occurred before taking action.
            # ----------------------------------------------------------------
            dist.barrier(device_ids=[int(os.environ["LOCAL_RANK"])])
            control = [bool(restructured), bool(training_complete), bool(pai_switch_error)]
            dist.broadcast_object_list(control, src=0)
            ...
            if restructured:
                # ------------------------------------------------------------
                # PAI added dendrites on rank 0.  The model graph has changed,
                # so non-main ranks cannot simply reuse their local copy.
                #
                # Strategy: rank 0 broadcasts the ENTIRE model (structure +
                # weights) to all other ranks. This ensures non-main ranks
                # have the exact same model architecture including any new
                ...
                model_for_pai = broadcast_full_model(model_for_pai, device, src=0)
        ...
        if restructured:
            if is_distributed:
                # All ranks now share the same model weights; safe to re-wrap.
                dist.barrier(device_ids=[int(os.environ["LOCAL_RANK"])])
        ...
        if is_best:
            ...
            if is_main_process:
                try:
                    model_to_save = model.module if isinstance(model, DDP) else model
                    save_on_master(
                        _state_dict_without_pai_metadata(model_to_save, disallowed_module_id=disallowed_module_id),
                        best_model_path,
                    )
                except Exception as exc:
                    print(f"Failed to save best model to {best_model_path}: {exc}")
        ...
        if best_validation_snapshot:
            epoch_log["flowers/test_accuracy_at_best_validation"] = best_validation_snapshot["test_accuracy_at_best_validation"]
            epoch_log["flowers/test_top5_at_best_validation"] = best_validation_snapshot["test_top5_at_best_validation"]
            epoch_log["flowers/epoch_at_best_validation"] = best_validation_snapshot["epoch_at_best_validation"]
    ...
    if is_main_process:
        ...
        if os.path.exists(best_model_path):
            try:
                _load_state_dict_compatible(model, best_model_path, device, disallowed_module_id=disallowed_module_id)
                model.to(device)
                print(f"Loaded best validation checkpoint from {best_model_path} for final test evaluation.")
            except Exception as exc:
                print(f"Failed to load best model from {best_model_path}: {exc}")
        ...
        if best_validation_snapshot:
            final_metrics["efficientnet_b5/accuracy_at_best_validation"] = best_validation_snapshot["test_accuracy_at_best_validation"]
            final_metrics["flowers/test_top5_at_best_validation"] = best_validation_snapshot["test_top5_at_best_validation"]
            final_metrics["flowers/validation_accuracy_best"] = best_validation_snapshot["validation_accuracy_best"]
            final_metrics["flowers/validation_top5_at_best_validation"] = best_validation_snapshot["validation_top5_at_best_validation"]
            final_metrics["flowers/epoch_at_best_validation"] = best_validation_snapshot["epoch_at_best_validation"]
